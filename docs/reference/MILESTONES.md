...
|---|---|---|---|---|
| 1 | Vinayak Brain dashboard is live in production | ✅ (confirm) | Pulse is the landing page (9 decision cards + morning brief); old overview moved to Explore; on Vercel/Railway/Supabase with CI | Deploy Sprint 1 and confirm Sandeep's workspace is the one connected and synced |
| 2 | Sandeep has used the dashboard ≥ 4 days/week for 60 consecutive days | 🟡 | `usage_events` + qualifying-week rule (unit-tested) + Milestone board; **Pulse landing page and 06:00 daily brief now give a reason to open it** | Set the tracked user on the board; onboard Sandeep; start the 60-day window by 1 Nov |
| 3 | AI chat answers a fixed 50-question evaluation set with ≥ 80% factual accuracy and 100% citation compliance | 🟡 | 29 golden cases; harness grades citation compliance (100% on the deterministic path), refusal, intent, bucket; CI ship-gate; `--record` writes runs to `eval_runs` for the board | Grow to **50 hand-verified questions**; add **expected values** to cases so *factual accuracy* is graded (today only intent/bucket/citation are); grade the **agent (native) path**, which is the production path, in CI; freeze the set with Shourya so "fixed" is auditable |
| 4 | Experiments log contains ≥ 30 logged experiments with outcomes captured | 🟡 | `experiments` table + API + Experiments page (propose → accept → run → close with outcome); counts on the Milestone board | Strategy watcher that *suggests* experiments (Sprint 2); start logging real ones now — 30 with outcomes needs ~6/month from October |
...
- Wiki: 🟡 `docs/wiki/` exists (dictionary, data sources, decisions, learnings, onboarding runbook, companies/); keep it current every sprint.
- Incident log: 🟡 `incidents` table + Milestone board logging; definitions in `docs/INCIDENTS.md`. The 60-day count is computed on the board.
