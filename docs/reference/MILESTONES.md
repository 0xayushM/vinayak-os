...
| 2 | Sandeep has used the dashboard ≥ 4 days/week for 60 consecutive days | 🟡 | `usage_events` recorded on every authenticated request; qualifying-week rule in `vinayak/usage.py` (unit-tested); Milestone board shows the run | Set the tracked user on the board; onboard Sandeep as a daily user; Pulse + brief (Sprint 1) so there is a reason to open it |
