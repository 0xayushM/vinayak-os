...
| **7 · Tools** | 🟠 SCAFFOLDED | `tools/contract.py` (`@tool`, side_effect, Evidence, quality), `tools/registry.py`, `tools/executor.py` (read=inline, non-read=propose to ledger, idempotency) | **Zero tools registered.** `routes/ai_tool.py` is a stub. No read-tool wraps of the query functions |
| **8 · Agent core** | 🔴 TO BUILD | — | Raw-SDK tool-use loop, grounding guard reuse, deterministic confidence authorizer |
| **9 · Action spine** | 🟠 SEEDED | `actions` + `events` tables exist (migration 008); executor writes proposals | Approval Inbox UI, email/WhatsApp executor, notifications, escalation, role-gated approval |
