...
_TOOLS: list[Tool] = [
    ...
    # Pulse — the derived-insight reads: a delta, a cause, or a decision.
    # These are what the morning brief and the landing page are built from, so
    # the agent can answer "what changed?" with the same numbers the owner saw.
    _make("pulse.get_aging_drift",
          "Receivables value that moved into the 61-90 and 90+ buckets over a window, and the customers behind it — 'is money sliding toward bad?'.",
          queries.get_aging_drift, params=("window_days",),
          inputs={"window_days": ToolInput(int, "Comparison window in days (default 30)", required=False)},
          ev_fields=[("bad_now", "In 61+ buckets", "money"), ("drift", "Drift", "money"),
                     ("drift_pct", "Drift share", "pct"),
                     ("total_outstanding", "Total outstanding", "money")]),
    _make("pulse.get_payment_behaviour",
          "Which customers are stretching payment — ranked on the current book, or on the change in days-to-pay once AR history allows.",
          queries.get_payment_behaviour,
          ev_fields=[("avg_days_to_pay", "Average days to pay", "days")]),
    _make("pulse.get_cash_30d",
          "Cash expected in (receivables due, plus part of the overdue book) against cash committed out (open purchase orders) over the next 30 days.",
          queries.get_cash_30d, params=("horizon_days",),
          inputs={"horizon_days": ToolInput(int, "Horizon in days (default 30)", required=False)},
          ev_fields=[("inflow", "Expected in", "money"), ("outflow", "Committed out", "money"),
                     ("net", "Net", "money"), ("overdue_total", "Overdue", "money")]),
    _make("pulse.get_week_delta",
          "This week's sales against the business's own trailing weekly average, with the customers and SKUs that account for the difference — 'why did revenue move?'.",
          queries.get_week_delta,
          ev_fields=[("this_week", "This week", "money"), ("usual_week", "Usual week", "money"),
                     ("delta", "Difference", "money"), ("delta_pct", "Change", "pct")]),
    _make("pulse.get_reorder_status",
          "Regular customers who are past 1.5x their own median gap between orders — who has quietly stopped ordering.",
          queries.get_reorder_status,
          ev_fields=[("flagged_count", "Customers flagged", "count"),
                     ("value_at_stake", "Typical order value at stake", "money")]),
    _make("pulse.get_concentration_trend",
          "Top-1 and top-3 customer revenue share now versus the previous equal period — is dependence growing?",
          queries.get_concentration_trend,
          ev_fields=[("top1_pct", "Top customer share", "pct"), ("top3_pct", "Top 3 share", "pct"),
                     ("top3_change", "Change in top-3 share", "pct")]),
    _make("pulse.get_dead_stock_delta",
          "Dead-stock value now versus 30 days ago — is trapped capital growing?",
          queries.get_dead_stock_delta,
          ev_fields=[("dead_value", "Dead stock", "money"), ("delta", "Change", "money"),
                     ("dead_count", "Dead SKUs", "count")]),
    _make("pulse.get_anomalies",
          "Things that should not happen: negative stock, an invoice far above a customer's normal, a vendor price jump, a stale feed.",
          queries.get_anomalies, source="meta", fresh_key="",
          ev_fields=[("count", "Anomalies", "count")]),
    _make("pulse.get_working_capital",
          "Where cash is locked: stock plus receivables, against what is committed on open purchase orders.",
          queries.get_working_capital,
          ev_fields=[("inventory", "Stock", "money"), ("receivables", "Receivables", "money"),
                     ("locked", "Locked up", "money"), ("open_commitments", "Committed", "money")]),
    _make("payments.get_inferred",
          "Payment dates learned by watching the receivables book empty out: who paid, when, and how many days it took.",
          queries.get_inferred_payments, params=("window_days",),
          inputs={"window_days": ToolInput(int, "Window in days (default 180)", required=False)},
          source="derived", fresh_key="",
          ev_fields=[("avg_days_to_pay", "Average days to pay", "days"),
                     ("median_days_to_pay", "Median days to pay", "days"),
                     ("count", "Payments observed", "count")]),
