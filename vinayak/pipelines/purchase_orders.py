...
class PurchaseOrderRow(BaseModel):
    ...
    def remap_api_fields(cls, data):
        ...
        # Immutable key only (value/status mutate across syncs → update in place).
        mapped["raw_id"] = stable_row_id(mapped["po_number"])
        return mapped
    ...
    def _upsert(self, conn, rows: list[PurchaseOrderRow], company_id: str) -> int:
        ...
        sql = """
            INSERT INTO tz_purchase_orders (
                company_id, raw_id, po_date, po_number, vendor_name, vendor_code,
